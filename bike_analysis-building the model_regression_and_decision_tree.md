...
```python
dfeatures_pred = pd.DataFrame(np.array(target_pred2).reshape(n_pred,1))
#print dfeatures_pred
```


```python
dftest_original['count']=dfeatures_pred
```


```python
output=pd.DataFrame(dftest_original[['datetime','count']])
```
...
```python
output.to_csv("Bike_Sharing_Dragomirescu_gini.csv", sep=',', header=True, index=False)
```
